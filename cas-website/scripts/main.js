...
//     map: new THREE.TextureLoader().load("../assets/img/stars.png"),
//     side: THREE.DoubleSide,
//     shininess: 0
// });
...
let satellite = (xco, yco, zco, color, facetype, nosetype, tailtype, size) => {
    ...
    if (nosetype == "c") {
        nose = new THREE.Mesh(
            new THREE.TorusGeometry( 0.04, 0.01, 8, 32), 
            mat2
        );
    } else if (nosetype == "g") {
        nose = new THREE.Mesh(
            new THREE.TorusKnotGeometry( 0.04, 0.01, 32, 8 ), 
            mat2
        );
    } else if (nosetype == "m") {
        nose = new THREE.Mesh(
...
for (let i = 1; i < sats.length; i++) {
    ...
    let satel = satellite(pos.x, pos.y, pos.z, col3, facetype, nosetype, tailtype, parseFloat(sats[i][3]));
    model.add(satel);

    //satel.children[0].geometry.computeBoundingBox();

    objectName(pos.x, pos.y - (Math.random() * (1.1 - 0.9) + 0.9), pos.z, 0.075, col3, sats[i][7]);

    let camposview = mapLatLon(lat, lon, 31);
    let posarrayview = {x: camposview.x, y: camposview.y, z: camposview.z}; 

    satObjects[i - 1] = {satellite: satel, site: sats[i][2], camPosView: posarrayview, city: sats[i][9], name: sats[i][7]};
};
...
for (let i = 1; i < sites.length; i++) {
    ...
    siteObjects[i - 1] = {siteObj: site, siteName: sites[i][0], country: sites[i][3], camPosSpace: posarrayspace};
};
...
for (let i = 1; i < cities.length; i++) {
    ...
    if (cities[i][0] == "Jakarta") {
        objectName(pos.x, pos.y + 0.3, pos.z - 0.08, 0.075, col3, " "+`${cities[i][0]}\n${cities[i][3]}`);
        ...
        objectName(pos.x, pos.y + 0.3, pos.z, 0.075, col3, `${cities[i][0]}\n`+"  "+`${cities[i][3]}`);
    } else if (cities[i][0].length < cities[i][3].length){
        objectName(pos.x, pos.y + 0.3, pos.z, 0.075, col3, "   "+`${cities[i][0]}\n${cities[i][3]}`);
    } else if (cities[i][0].length == cities[i][3].length){
        objectName(pos.x, pos.y + 0.3, pos.z, 0.075, col3, `${cities[i][0]}\n${cities[i][3]}`);
    ...
    cityObjects[i - 1] = {cityObj: place, city: cities[i][0], camPosSpace: posarrayspace, country: cities[i][3]};
};
...
siteObjects.forEach((d) => {
    ...
    citylinkObjects.push({obj: con, country: d["country"], site: d["siteName"]});
});
